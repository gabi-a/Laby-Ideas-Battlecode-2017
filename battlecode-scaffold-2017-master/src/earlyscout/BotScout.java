	...
	public static void turn(RobotController rc) throws GameActionException {
		...
		for (int i = 0; i < enemies.length; i++) {
			if (enemies[i].getType() == RobotType.GARDENER || rc.getRoundNum() > 500) {
				// Comms.writeStack(rc, Comms.ENEMY_ARCHON_START,
				// Comms.ENEMY_ARCHON_END, enemies[i].getLocation());
				enemyTarget = enemies[i];
			} else {
				// Comms.writeStack(rc, Comms.ENEMY_START, Comms.ENEMY_END,
				// enemies[i].getLocation());
			}
		}
